...
: FM()
{
	...
	for ( unsigned int i=0; i<3; i++ )
		waves_[i] = new WaveLoop( (Stk::rawwavePath() + "sinewave.ric").c_str(), true );
	waves_[3] = new WaveLoop( (Stk::rawwavePath() + "fwavblnk.ric").c_str(), true );
...
StkFloat Rhodey :: computeSample()
{
	...
	gains_[3] * adsr_[3]->tick(); // waves_[3]->tick();
